...
# TODO: understand and reuse following patterns
DOC_PATTERN_URL = re.compile(
    r"example|tutorial|doc|api|quickstart|note|guide|blog|using|reference|\.io|sample|demo|template",
    re.I,
)
DOC_PATTERN_CODE = re.compile(r"example", re.I)
ERROR_PATTERN = re.compile(
    r"exception|error|warning|404|can't|can\s{0,1}not|could\s{0,1}not|un[a-z]{3,}", re.I
)
ISSUE_PATTERN = re.compile(r"issue|pull|#[0-9]+|[0-9]{4,}", re.I)
TAG_PATTERN = re.compile(r"CODETAG|APITAG")
CODE_PATTERN = re.compile(
    r"[=;%$/<>\{\}\[\]]|public\sstatic\s(void){0,1}(\smain){0,1}|String"
)

NUM_PATTERN = re.compile(
    r"^(([^a-uwyz]+?\d[^a-uwyz]*(beta[0-9]+){0,1})|(\s*beta[0-9]+))\s*$", re.I
)
...
def replace_tokens_simple(content):
    ...
    MAX_API_LENGTH = 150
    # * use markdown pattern to simply get content block
    ...
    content = re.sub(
        r"\S+?(Error|Exception)([^A-Za-z\s]\S*|\s|$)|404", " ERRORTAG ", content
    )
    ...
    for _ in re.finditer(
        r"\s(\S+?\.(ml|xml|png|csv|jar|sh|sbt|zip|exe|md|txt|js|yml|yaml|json|sql|html|pdf|jsp|php|prod|scss|ts|jpg|png|bmp|gif))[?,\.]{0,1}\s",
        content,
        re.I,
    ):
        file = _.group(1)
    ...
    content = re.sub(
        r"[^a-uwyz]+?\d[^a-uwyz]*(beta[0-9]+){0,1}|beta[0-9]+",
        " NUMBERTAG ",
        content,
        flags=re.I,
    )
