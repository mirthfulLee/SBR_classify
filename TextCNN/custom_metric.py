...
class RootF1Metric(Metric):
    def __init__(self, thres=0.5) -> None:
        super().__init__()
        self.reset()
        self._thres = thres
    ...
    def get_best_threshold(self, interval=[0.3, 0.9]):
    ...
    def __call__(self, predictions: torch.Tensor, gold_labels: torch.Tensor, mask: Optional[torch.BoolTensor]=None):
        self.root_pred = predictions if self.root_pred is None else torch.cat((self.root_pred, predictions))
